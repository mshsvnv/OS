...
int main(int argc, char *argv[])
{
    ...
    tinfo.thread_num = 1;
    tinfo.argv_string = NULL;
    s = pthread_create(&tinfo.thread_id, &attr, thr_fn, NULL);
    if (s != 0)
    {
    ...
    if (s != 0)
	{
        syslog(LOG_ERR, "pthread_attr_destroy");
    }
    time_t raw_time;
    for (;;)
    {
        sleep(5);
		time(&raw_time); // текущая дата в секундах
		syslog(LOG_INFO, "Current time is: %s", ctime(&raw_time));
    }
}
